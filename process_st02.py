...
def execute(iDir, oDir, pDir, gpu_id):
    ...
    for ev in pth_cfg["process"]:        
        ...
        cmdline  += ' -iSfx {} '.format(os.path.normpath("upsampled"))
        cmdline  += ' -iDir {} '.format(os.path.normpath(iDir))
        cmdline  += ' -pth {} '.format(os.path.normpath(pDir+"/"+pth_name))
        cmdline  += ' -sL '     # we need Labels
        cmdline  += ' -sB '     #   and box
        cmdline  += ' -fH '     # fill holes in ROI
        cmdline  += ' -plim 1 ' # limit number of ROI polygons to 1
...
def main():
    ...
    session_l = expand_session_dirs(args.session_id, stage01_dir)
    ...
    for ses in session_l:
        ...
        imgDir 	    = os.path.normpath(os.path.join(stage01_dir, ses))
        outDir 		= os.path.normpath(os.path.join(stage02_dir, ses))
        ...
        execute(imgDir, outDir, "as_pth/st02_pth", 0)
